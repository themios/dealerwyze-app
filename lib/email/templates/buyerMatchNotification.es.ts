/**
 * Spanish email template for buyer match notifications
...
export function buildBuyerMatchNotificationHtmlEs(
  agentName: string,
  appUrl: string,
  matches: MatchItem[]
): string {
  ...
  const pluralMatch = matchCount === 1 ? 'coincidencia' : 'coincidencias'
  ...
  <tr><td>
    <div style="background:#0D2B55;border-radius:12px 12px 0 0;padding:24px 32px">
      ...
      <h1 style="margin:8px 0 0;color:#FFFFFF;font-size:20px;font-weight:700">
        ${matchCount} nueva ${pluralMatch}
      </h1>
    </div>
    <div style="background:#FFFFFF;padding:32px;border:1px solid #E2E8F0;border-top:none">
      <p style="margin:0 0 24px;color:#374151;font-size:15px;line-height:1.7">
        Hola ${agentName}, encontramos ${matchCount} propiedad${matchCount !== 1 ? 'es' : ''} que coinciden con los criterios de tu comprador.
        Revisa ${matchCount === 1 ? 'la' : 'las'} a continuación y comunícate con tus compradores que podrían estar interesados.
      </p>
      ...
      <div style="background:#F0F7FF;border:1px solid #BFE7FF;border-radius:8px;padding:24px;margin:0 0 28px">
        <p style="margin:0 0 16px;font-size:13px;font-weight:700;color:#0D2B55;text-transform:uppercase;letter-spacing:0.05em">
          Nueva${matchCount === 1 ? '' : 's'} ${pluralMatch}
        </p>
        <table cellpadding="0" cellspacing="0" width="100%">
          ${matchRows}
        </table>
      ...
      <div style="text-align:center;margin-bottom:28px">
        <a href="${appUrl}/es/app/matches"
          ...
          Revisar Tus Coincidencias
        </a>
      </div>

      <p style="margin:0 0 8px;font-size:14px;color:#374151;line-height:1.7">
        Puedes administrar todas tus coincidencias de compradores desde tu panel. Márcalas como enviadas, revisadas o cerradas.
      </p>
      <p style="margin:0;font-size:14px;color:#374151;line-height:1.7">
        Las nuevas propiedades continuarán coincidiendo automáticamente durante todo el día.
      </p>
