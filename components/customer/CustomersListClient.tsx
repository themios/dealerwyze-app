...
import { Phone, CheckSquare, Square, X, UserCheck, Archive } from 'lucide-react'
...
export default function CustomersListClient({ customers: initial, isAdmin, agents, lastActivityMap = {}, showArchived = false }: Props) {
  ...
  return (
    <>
      ...
      {sorted.length > 0 && (
        <div className={selectMode ? "px-4 py-2 space-y-2" : "divide-y divide-border bg-card border rounded-xl mx-3 my-2 overflow-hidden"}>
          {sorted.map(customer => {
              ...
              <div key={customer.id} className="flex items-center hover:bg-accent/40 transition-colors">
                <Link href={`/customers/${customer.id}`} className="flex items-center gap-3 px-4 py-2.5 flex-1 min-w-0">
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0" suppressHydrationWarning>
                    <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${ageBadge.cls}`}>
                      {ageBadge.label}
                    </span>
                    <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${contactBadge.cls}`}>
                      {contactBadge.label}
                    </span>
                  </div>
                </Link>
                {/* Archive button — hidden in archived view */}
                {!showArchived && (
                  archiveConfirm === customer.id ? (
                    <div className="flex items-center gap-1 pr-2 flex-shrink-0">
                      ...
                      />
                      <button
                        onClick={e => { e.stopPropagation(); handleArchive(customer.id) }}
                        className="text-xs text-destructive font-medium px-1.5"
                      >
                        Archive
                      </button>
                      <button
                        onClick={e => { e.stopPropagation(); setArchiveConfirm(null); setArchiveReason('') }}
