...
import { buildNarrationScript } from '@/lib/remotion/generateNarration'
...
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const [{ data: v }, { data: orgSettings }, { data: org }, { data: videoSettings }] = await Promise.all([
    userSb.from('vehicles').select('year, make, model, trim, price, mileage, color, interior, engine, mpg_city, mpg_hwy, vin, features, title_status').eq('id', vehicleId).single(),
    userSb.from('org_settings').select('dealer_name, phone, city, state, website').eq('org_id', profile.org_id).maybeSingle(),
    userSb.from('organizations').select('name').eq('id', profile.org_id).single(),
