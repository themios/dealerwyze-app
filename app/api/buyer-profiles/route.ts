...
type CreateBuyerProfile = z.infer<typeof CreateBuyerProfileSchema>;
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Bedrooms min must be <= max' },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Bathrooms min must be <= max' },
        { status: 400 }
      );
    }
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Year built min must be <= max' },
        { status: 400 }
      );
    }
