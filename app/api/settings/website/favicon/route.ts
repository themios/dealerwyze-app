/**
 * POST   /api/settings/website/favicon — site icon (JPEG/PNG/WebP, max 512KB)
 * DELETE — clear custom favicon
 */
...
export async function POST(req: NextRequest) {
  ...
  if (file.size > MAX_BYTES) {
    return NextResponse.json({ error: 'Favicon must be 512 KB or smaller.' }, { status: 413 })
  }
  ...
  const storageKey = `${org.id}/website-favicon-${crypto.randomUUID()}.${ext}`

  await removeStored(service, org.id, org.website_favicon_url ?? null)
  ...
  if (uploadErr) {
    console.error('[website/favicon POST]', uploadErr.message)
  ...
  const { error: updateErr } = await supabase
    .from('organizations')
    .update({ website_favicon_url: publicUrl, updated_at: new Date().toISOString() })
    .eq('id', org.id)

  if (updateErr) {
    await service.storage.from(BUCKET).remove([storageKey])
    return NextResponse.json({ error: 'Could not save favicon URL' }, { status: 500 })
  }
  ...
  void logOrgAudit({
    ...
    actor_id: profile.id,
    actor_type: 'user',
    action: 'website_favicon_uploaded',
    details: { bytes: bytes.byteLength, mime: sniffed },
    ip: requestClientIp(req),
  })

  return NextResponse.json({ website_favicon_url: publicUrl })
}
...
export async function DELETE(req: NextRequest) {
  ...
  if (error) {
    return NextResponse.json({ error: 'Could not clear favicon' }, { status: 500 })
  }
  ...
  void logOrgAudit({
    ...
    actor_id: profile.id,
    actor_type: 'user',
    action: 'website_favicon_removed',
    ip: requestClientIp(req),
  })

  return NextResponse.json({ ok: true, website_favicon_url: null })
}
